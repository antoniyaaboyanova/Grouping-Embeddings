...
def run_blip_model_exp(project_dir, 
                  image_dir, 
                  input_file_name = "fmri_train_stim.npy", 
                  output_file_name = "fmri_train_caps_blip.npy"):
    ...
    for im in tqdm(fmri_stim):
        imagePaths.append(os.path.join(image_dir, im))
...
def run_llava_model(project_dir, 
    ...
    for i in tqdm(range(0, len(imagePaths), batch_size)):
        ...
        # Generate the output
        output = model.generate(**inputs, max_tokens = 40)
...
def run_llava_model_exp(project_dir, 
                    image_dir, 
                    input_file_name = "exp_images.npy",
                    output_file_name = "exp_caps_llava.npy"):
    ...
    # Process the current batch
    batch_images = [Image.open(imagePaths[j]).convert('RGB') for j in range(len(imagePaths))]
    inputs = processor(text=prompts * len(batch_images), images=batch_images, padding=True, return_tensors="pt").to("cuda")
...
#### CLIP extractors #####
def clip_vis(project_dir, image_dir, input_file_name = "fmri_train_stim.npy"):
    
    fmri_stim = np.load(os.path.join(project_dir, "files", input_file_name), allow_pickle=True)
    imagePaths = []

    if "fmri" in input_file_name:
        output_file_name =  "CLIP_vis_fmri.pickle"
        for im in tqdm(fmri_stim):
            im_cat = im.split(".")[0][0:-4]
            imagePaths.append(os.path.join(image_dir, im_cat, im))

    else:
         output_file_name = f"CLIP_vis_{input_file_name.split('.')[0]}.pickle"
         for im in tqdm(fmri_stim):
            imagePaths.append(os.path.join(image_dir, im))
...
def clip_txt(project_dir, image_dir,  language_model="blip", input_file_name = "fmri_train_stim.npy", cap_name=None):
    fmri_stim = np.load(os.path.join(project_dir, "files", input_file_name), allow_pickle=True)
    imagePaths = []
    
    # If cap_name is not provided, use the default format
    if cap_name is None:
        cap_name = f"fmri_train_caps_{language_model}.npy"
        output_file_name = f"CLIP_txt_fmri_{language_model}.pickle"
        
        for im in tqdm(fmri_stim):
            im_cat = im.split(".")[0][0:-4]
            imagePaths.append(os.path.join(image_dir, im_cat, im))
    else:
        output_file_name = f"CLIP_txt_{input_file_name.split('.')[0]}_{language_model}.pickle" 
        for im in tqdm(fmri_stim):
            imagePaths.append(os.path.join(image_dir, im))
    ...
    if language_model == "blip":
        ...
        CLIP_txt = {"stimuli": fmri_stim,
                    "stimuli_paths": imagePaths,
                    "captions": captions[:,0],
                    "embeddings": embeddings_1.reshape(len(embeddings_1), -1)}
